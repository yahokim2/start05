# command ex) python check_person_face15.py --pretrained pretrained/ram_plus_swin_large_14m.pth --interval 03 
import argparse
import torch
import os
import cv2
...
def process_webcam_input(model, device, transform, interval, csv_file, face_model, feature_extractor, reference_embeddings, reference_names):
    ...
    while True:
        ...
        if current_time - last_processed_time > interval:  # interval 초마다 처리
            ...
            # interval(3초) 간격 후 다시 사람 수를 인식하도록 반복
            time.sleep(interval)
...
# 추가=== 4. Yuzlar ma'lumotini saqlash va terminalda chiqarish ===
